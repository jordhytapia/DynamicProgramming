...
{
    std::unique_ptr<ArmorVector> newSource(new ArmorVector);
    
    for(int i = 0; i < source.size(); ++i)
    {
        if(source[i]->defense() > min_defense && source[i]->defense() <= max_defense)
            newSource->push_back(source[i]);
        
        if(newSource->size() == total_size)
            break;
    }
...
)
{
    ...
    for(int i = 1; i < map.size(); ++i)
    {
        ...
        for(int j = 1; j < map[i].size(); ++j)
        {
            ...
            if(j >= c && map[i-1][j-c] + d > map[i][j])
                map[i][j] = map[i-1][j-c] + d;
        }
    }
    
    int cap = map[0].size()-1;
    ...
    for(size_t i = map.size()-1; i > 0; --i)
    {
        if(map[i][cap] != map[i-1][cap])
        {
            ...
            selected->push_back(armors[itemIndex]);
